...
- `0-Core`: `BisCore` — all physical elements ultimately subclass from here
- `0-Core`: `Analytical` — for loading analysis elements
- `0-Core`: `Functional` — for circuit/feeder topology
- `1-Common`: `DistributionSystems` (alias `dsys`) — primary mixin source for
  flow/control/sensor roles
- `1-Common`: `SpatialComposition` — for geographic hierarchy
  (circuit → section → span)
- `1-Common`: `LinearReferencing` — for feeder milepost addressing
...
> **Version note for Claude Code:** Verify the exact released versions of each
> referenced schema against the current `iTwin/bis-schemas` repo before
> finalizing the schema references. The versions above are approximate.
...
Subclass of `bis:PhysicalElement`. Implements `dsys:IDistributionFlowElement`.
...
All direct pole attachments subclass `edist:PoleAttachment` → `bis:PhysicalElement`.
...
Subclass of `bis:PhysicalElement`. Abstract.
...
Subclass of `bis:PhysicalElement`. Associated to the pole, not a subtype of
...
This schema lives in its own public repo. The UPM implementation remains
private. The repo structure should mirror what `iTwin/bis-schemas` expects
when the PR is opened:

```
bis-schema-electric-distribution/        ← this public repo
├── README.md                            # Proposal overview, scope, rationale
├── BIS_SCHEMA_DESIGN.md                 # This file
├── ElectricDistribution.ecschema.xml    # The schema being authored
├── Released/                            # Empty until first release PR
├── docs/
│   ├── class-diagrams.md               # Mermaid class diagrams
│   ├── upm-mapping.md                  # UPM field → BIS property crosswalk
│   │                                   # (references private repo by path,
│   │                                   #  not linked publicly)
│   ├── spidacalc-mapping.md            # SPIDAcalc JSON → BIS crosswalk (Bentley)
│   ├── plspole-mapping.md              # PLS-POLE → BIS crosswalk (Bentley)
│   ├── ocalc-mapping.md                # O-Calc Pro → BIS crosswalk (Osmose)
│   └── rationale.md                    # Design decisions and tradeoffs
├── tests/
│   └── validation/                     # BIS rule validation fixtures
└── CHANGELOG.md
```

**Private reference implementation** (not in this repo, local only):
```
C:\Users\JJGIV\PycharmProjects\highvoltage_agents\
└── pole_loading_analyst_agent\
    └── domain\                         ← UPM domain type definitions
...
These notes help Claude Code author the mapping documents in `docs/` and inform
connector implementation decisions.
...
### UPM (Universal Pole Model — private)
- **Format:** Python/Pydantic models, serializable to JSON
- **Location:** `C:\Users\JJGIV\PycharmProjects\highvoltage_agents\pole_loading_analyst_agent\domain\`
- **Role:** Canonical internal representation all three external formats translate
  into before analysis. Primary reference for the BIS schema.

---

## Connector Integration Notes

When building the UPM → iModel connector in the private repo using
